...
public class PVPHubScreen extends Screen {
     ...
     *  2=reject-req, 3=unfriend, 4=add-friend-event.button(), 5=expand-toggle. */
    ...
    /** Captured per-frame for the back-event.button() hit rect on the detail view. */
     ...
     *  click flips the event.button() to "Confirm?" and starts the timer below. The
    ...
    private void renderQueueTab(GuiGraphics g, int mx, int my, int px, int y, int panelW, int panelH) {
        // Layout:
        //   right column = 3D viewer + hover label + queue/accept event.button() (stacked)
        //   left column  = ranked/scope toggles + kit list + loadout editor (full height)
        int viewerX = px + panelW - VIEWER_W - 8;
        int viewerY = y + 8;
        // Reserve 36px at the bottom of the right column for the queue/accept event.button().
        int rightBtnH = 36;
        int viewerH = panelH - 16 - rightBtnH;

        // Left column extents, no event.button() reserve here anymore, loadout fills.
        int leftAreaX = px + 8;
        ...
        if (matchmaking.state() == MatchmakingService.State.MATCH_FOUND) {
            ...
            // Row 3: Loadout editor, fills remaining vertical space (queue event.button() is
        ...
        // Queue event.button() / status, centered in right column below the 3D viewer
        ...
        if (ms == MatchmakingService.State.IDLE) {
            ...
            // for ~6s above the QUEUE UP event.button() so the user knows why their click was rejected.
            ...
            // event.button() is a soft "skip", clicking accepts immediately and lets
     ...
     * slot present in the current kit's by_slot, each row a clickable event.button() that opens
     * the variant picker.
     */
    ...
    /** Top-of-tab pill event.button() used for Live/History + Self/Everyone toggles. */
    ...
    /** Refresh-event.button() hit rect (Live Duels). Set each render in renderLiveDuels. */
    ...
    private void renderLiveDuels(GuiGraphics g, int mx, int my, int px, int y, int panelW, int panelH) {
        // Refresh event.button(), top-right of the Live Duels area. CD is implicit
    ...
    /** Called from the SPECTATE event.button() click. POSTs to /duels/{id}/spectate
    ...
    private void renderFriendRow(GuiGraphics g, int mx, int my,
        ...
        // "X is offline" toast instead of clicking a phantom invisible event.button().
        int invX = x + w - 90;
    ...
    private void renderPlayerDetailView(GuiGraphics g, int mx, int my,
        ...
        // Hint line above the event.button() row. Always show a clickable "Buy more"
        ...
        try {
            ...
            if (json.has("properties") && json.get("properties").isJsonArray()) {
                ...
                for (var el : arr) {
                    ...
                    // profile.getProperties() not accessible on 1.21.9 — skip property attachment
                    // skinManager.get() still does its own Mojang lookup for the UUID
                    propCount++;
                }
            }
    ...
    /** Smaller pill event.button() for the scope picker (Local/Region/Global), 48px wide. */
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (activeTab == Tab.QUEUE) {
            ...
            // through to the queue-event.button() / 3D-viewer logic below.
            if (matchmaking.state() == MatchmakingService.State.MATCH_FOUND
            ...
            // Queue / accept event.button() (centered in right column below the viewer)
            int rightBtnH2 = 36;
